...
def str_to_scipy_sparse_feat(mat_str):
    """
    Convert the string in input into a scipy sparse coo matrix. It uses a
    custom str encoding to save space and it's particularly suited for
    sparse matrices, like the graph adjacency matrix.
...
def get_bart_feature(bart_str):
    """
    Get the numpy matrix of feature (list type)

    Args:
        bart_str: list type of the nodes feature

    Return
        numpy matrix
    """
    opc_array = np.array(bart_str)
    return opc_array
...
def pack_batch(graphs, features, use_features, nofeatures_size=1):
    ...
    for i, d in enumerate(zip(graphs, features)):
        ...
        for edge in list(g.edges(data=True)):
            if edge[2]['type'] == 0:
                edge_feature.append([1, 0, 0])
            elif edge[2]['type'] == 1:
                edge_feature.append([0, 1, 0])
            elif edge[2]['type'] == 2:
                edge_feature.append([0, 0, 1])
    ...
    return GraphData(
        ...
        # edge_features=np.ones((n_total_edges, 8), dtype=np.float32),
