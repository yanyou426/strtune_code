import json
import math
import networkx as nx
import pandas as pd

from .graph_factory_base import GraphFactoryBase
from .graph_factory_utils import *
from tqdm import tqdm

import logging
log = logging.getLogger('gnn')


class GraphFactoryInference(GraphFactoryBase):

    def __init__(self, func_path, feat_path, batch_size,
                 use_features, features_type, bb_features_size, bertmodel):
        """
            Args:
                func_path: CSV file with function pairs
                feat_path: JSON file with function features
                batch_size: size of the batch for each iteration
                use_features: if True, load the graph node features
                features_type: used to select the appropriate decoder and data
                bb_features_size: numer of features at the basic-block level
        """
        if batch_size <= 0 or batch_size % 2 != 0:
            raise SystemError("Batch size must be even and >= 0")

        self._batch_size = batch_size
        log.info("Batch size (inference): {}".format(self._batch_size))

        self._use_features = use_features
        self._features_type = features_type
        self._bb_features_size = bb_features_size
        # self._decoder = get_bart_feature
        self._decoder = str_to_scipy_sparse
        self._decoder_feat = str_to_scipy_sparse_feat
        self._bertmodel = bertmodel

        # Load function pairs
        log.info("Reading {}".format(func_path))
        self._func = pd.read_csv(func_path)

        # Load function features
        log.info("Loading {}".format(feat_path))
        with open(feat_path) as gfd_in:
            self._fdict = json.load(gfd_in)

        # Initialize the iterator
        self._get_next_pair_it = self._get_next_pair()

        # Number of function pairs
        self._num_func_pairs = self._func.shape[0]
        log.info("Num func pairs (inference): {}".format(self._num_func_pairs))

        # _get_next_pair() returns a pair of functions
        # _batch_size must be even and >= 2
        # _num_batches is the number of iterations to cover the input data
        # Example:
        #   * 100 functions. Batch_size = 20; 5 iterations
        #   * 100 functions. Batch_size = 16; 7 iterations
        self._num_batches = math.ceil(
            self._num_func_pairs / self._batch_size)
        log.info("Num batches (inference): {}".format(self._num_batches))

    def _get_next_pair(self):
        """The function implements an infinite loop over the input data."""
        while True:
            log.info("(Re-)initializing the iterators")
            # (Re-)initialize the iterators
            iterator = self._func.iterrows()

            for _ in range(self._num_func_pairs):
                # Get the next row
                r = next(iterator)[1]
                # Get the features for the left function
                f_l = self._fdict[r['idb_path_1']][r['fva_1']]
                # ... and for the right one
                f_r = self._fdict[r['idb_path_2']][r['fva_2']]
                
                f_l_cfg = nx.DiGraph(str_to_scipy_sparse(f_l['cfg']))
                f_l_ddg = nx.DiGraph(str_to_scipy_sparse(f_l['ddg']))
                f_l_g = nx.MultiDiGraph()
                for node in f_l_cfg.nodes():
                    if f_l_cfg.out_degree(node) > 1:
                        successors = list(f_l_cfg.successors(node))
                        for successor in successors:
                            f_l_g.add_edge(node, successor, type=1)
                    elif f_l_cfg.out_degree(node) == 1:
                        successor = list(f_l_cfg.successors(node))[0]
                        f_l_g.add_edge(node, successor, type=0)
                        
                for edge in f_l_ddg.edges():
                    f_l_g.add_edge(edge[0], edge[1], type=2)
                    
                # for node in f_l_cfg.nodes():
                #     f_l_g.add_node(node)
            
                f_r_cfg = nx.DiGraph(str_to_scipy_sparse(f_r['cfg']))
                f_r_ddg = nx.DiGraph(str_to_scipy_sparse(f_r['ddg']))
                f_r_g = nx.MultiDiGraph()
                for node in f_r_cfg.nodes():
                    if f_r_cfg.out_degree(node) > 1:
                        successors = list(f_r_cfg.successors(node))
                        for successor in successors:
                            f_r_g.add_edge(node, successor, type=1)
                    elif f_r_cfg.out_degree(node) == 1:
                        successor = list(f_r_cfg.successors(node))[0]
                        f_r_g.add_edge(node, successor, type=0)
                        
                for edge in f_r_ddg.edges():
                    f_r_g.add_edge(edge[0], edge[1], type=2)
                    
                # for node in f_r_cfg.nodes():
                #     f_r_g.add_node(node)

                if self._use_features:
#                     if self._features_type == 'roberta':
#                         # roberta
#                         stranddict = f_l['strandinfo']
#                         strands = []
#                         avg = []
#                         for add in sorted(stranddict):
#                             if add == '0':
#                                 continue
#                             s = stranddict[add]
                            
#                             inst = ""
#                             for ins in s:
#                                 ops = ins.replace(', ', ' ')
#                                 inst += (" " + ops)
#                             inst = inst.lstrip(' ')
#                             strands.append(inst[:510])
                            
#                             # inst = ""
#                             # for strand in s:
#                             #     if inst != "":
#                             #         inst += ';'
#                             #     inst += strand
#                             # strands.append(inst[:510])
                            
#                             # if add == '0':
#                             #     strands = ['']
#                             # embed = self._bertmodel.encode(strands)
#                             # embed = np.mean(embed, axis=0)
#                             # avg.append(embed)
                            
#                         embed1 = self._bertmodel.encode(strands)
#                         # embed1 = np.array(avg)
                        
#                         stranddict = f_r['strandinfo']
#                         strands = []
#                         avg = []
#                         for add in sorted(stranddict):
#                             if add == '0':
#                                 continue
#                             s = stranddict[add]
#                             inst = ""
#                             for strand in s:
#                                 if inst != "":
#                                     inst += ';'
#                                 inst += strand
#                             strands.append(inst[:510])
                            
#                             # if len(strands) == 0:
#                             #     s = ['']
#                             # embed = self._bertmodel.encode(s)
#                             # embed = np.mean(embed, axis=0)
#                             # avg.append(embed)
                            
#                         embed2 = self._bertmodel.encode(strands)
#                         # embed2 = np.array(avg)
                    
                    # else:
                        # microcode IR
                    embed1 = self._decoder_feat(f_l[self._features_type])
                    embed2 = self._decoder_feat(f_r[self._features_type])
                 
                    
                        
                    yield (
                        (
                            # nx.DiGraph((str_to_scipy_sparse(f_l['cfg'])[1:])[:, 1:]),
                            # nx.DiGraph((str_to_scipy_sparse(f_r['cfg'])[1:])[:, 1:])
                            # nx.DiGraph(str_to_scipy_sparse(f_l['cfg'])),
                            # nx.DiGraph(str_to_scipy_sparse(f_r['cfg']))
                            f_l_g,
                            f_r_g
                        ),
                        (
                            # self._decoder(f_l[self._features_type])[1:],
                            # self._decoder(f_r[self._features_type])[1:]
                            embed1,
                            embed2
                        )
                    )
                else:
                    yield (
                        (
                            # nx.DiGraph((str_to_scipy_sparse(f_l['cfg'])[1:])[:, 1:]),
                            # nx.DiGraph((str_to_scipy_sparse(f_r['cfg'])[1:])[:, 1:])
                            # nx.DiGraph(str_to_scipy_sparse(f_l['cfg'])),
                            # nx.DiGraph(str_to_scipy_sparse(f_r['cfg']))
                            f_l_g,
                            f_r_g
                        ),
                        (
                            -1, -1
                        )
                    )

    def pairs(self):
        """Yields batches of pair data."""
        for _ in tqdm(range(self._num_batches),
                      total=self._num_batches):
            batch_graphs = list()
            batch_features = list()

            for _ in range(self._batch_size):
                g_pair, f_pair = next(self._get_next_pair_it)
                batch_graphs.append((g_pair[0], g_pair[1]))
                batch_features.append((f_pair[0], f_pair[1]))

            # Pack everything in a graph data structure
            packed_graphs = pack_batch(
                batch_graphs,
                batch_features,
                self._use_features,
                nofeatures_size=self._bb_features_size)

            yield packed_graphs

    def triplets(self):
        """ Yields batches of triplet data. For training only."""
        pass
    
