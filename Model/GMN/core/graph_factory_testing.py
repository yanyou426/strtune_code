...
class GraphFactoryTesting(GraphFactoryBase):
    ...
    def __init__(self, pos_path, neg_path, feat_path, batch_size,
        ...
        log.info("Reading {}".format(pos_path))
        self._func_pos = pd.read_csv(pos_path)
        log.info("Reading {}".format(neg_path))
    ...
    def _get_next_pair(self):
        ...
        while True:
            ...
            for _ in range(self._num_func_pairs):
                ...
                gs = list()
                keys = [f_pl, f_pr, f_nl, f_nr]
                for key in keys:
                    cfg = nx.DiGraph(str_to_scipy_sparse(key['cfg']))
                    ddg = nx.DiGraph(str_to_scipy_sparse(key['ddg']))
                    g = nx.MultiDiGraph()
                    for node in cfg.nodes():
                        if cfg.out_degree(node) > 1:
                            successors = list(cfg.successors(node))
                            for successor in successors:
                                g.add_edge(node, successor, type=1)
                        elif cfg.out_degree(node) == 1:
                            successor = list(cfg.successors(node))[0]
                            g.add_edge(node, successor, type=0)
                            
                    for edge in ddg.edges():
                        g.add_edge(edge[0], edge[1], type=2)
                    # for node in cfg.nodes():
                    #     g.add_node(node)
                    gs.append(g)
...
#                         # roberta
#                         l = [f_pl, f_pr, f_nl, f_nr]
#                         embeds = []
#                         avg = []
#                         for i in range(4):
#                             stranddict = l[i]['strandinfo']
#                             strands = []
...
#                         yield (
#                             (
#                                 # nx.DiGraph(str_to_scipy_sparse(f_pl['cfg'])),
#                                 # nx.DiGraph(str_to_scipy_sparse(f_pr['cfg'])),
#                                 gs[0],
#                                 gs[1]
#                             ),
#                             (
#                                 # self._decoder(f_pl[self._features_type])[1:],
#                                 # self._decoder(f_pr[self._features_type])[1:]
#                                 embeds[0],
#                                 embeds[1]
#                             ),
#                             (
#                                 # nx.DiGraph(str_to_scipy_sparse(f_nl['cfg'])),
#                                 # nx.DiGraph(str_to_scipy_sparse(f_nr['cfg']))
#                                 gs[2],
#                                 gs[3]
#                             ),
#                             (
#                                 # self._decoder(f_nl[self._features_type])[1:],
#                                 # self._decoder(f_nr[self._features_type])[1:]
#                                 embeds[2],
#                                 embeds[3]
#                             )
#                         )
                    # else:
                    fpl = self._decoder_feat(f_pl[self._features_type])
                    fpr = self._decoder_feat(f_pr[self._features_type])
                    fnl = self._decoder_feat(f_nl[self._features_type])
                    fnr = self._decoder_feat(f_nr[self._features_type])
