...
IDA_PLUGIN = join(dirname(abspath(__file__)), 'gen_mc_list.py')
...
@click.option("-o", "--output-txt", required=True,
              help="Path to the output txt file")
def main(idbs_folder, output_txt):
    """Call gen_mc_list.py IDA script."""
    try:
        print("[D] IDBs folder: {}".format(idbs_folder))
        print("[D] Output CSV: {}".format(output_txt))
